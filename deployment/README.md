...
```yaml
# deployment/docker_compose/docker-compose.override.yml
services:
  nginx:
    ports:
      - "80:80"
...
cp env.template .env        # then fill in the values from the list above
docker compose up -d
```

Set `WEB_DOMAIN` to the URL you actually browse (`http://<host>` without TLS).
